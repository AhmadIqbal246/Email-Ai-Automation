...
const AdminDashboard = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const response = await fetch(`${ENV.API_BASE_URL}/api/admin/employees/`, {
        headers: {
  ...
  const fetchInvitations = async () => {
    try {
      const response = await fetch(`${ENV.API_BASE_URL}/api/admin/invitations/`, {
        headers: {
          'Authorization': `Token ${localStorage.getItem('authToken')}`
        }
      })
  ...
  const handleInviteEmployee = async (e) => {
    ...
    try {
      const response = await fetch(`${ENV.API_BASE_URL}/api/admin/send-invitation/`, {
        method: 'POST',
        headers: {
  ...
  const resendInvitation = async (invitationId) => {
    try {
      const response = await fetch(`${ENV.API_BASE_URL}/api/admin/resend-invitation/${invitationId}/`, {
        method: 'POST',
        headers: {
          'Authorization': `Token ${localStorage.getItem('authToken')}`
        }
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="w-8 h-8 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-full flex items-center justify-center">
                <span className="text-white text-sm font-medium">{(user?.first_name?.[0] || user?.email?.[0] || 'A').toUpperCase()}</span>
              </div>
              <button 
